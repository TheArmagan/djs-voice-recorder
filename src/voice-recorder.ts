...
export class VoiceRecorder {
    ...
    constructor(options: Partial<RecordOptions> = {}) {
        this.options = {
            maxUserRecordingLength: (options.maxUserRecordingLength ?? 120) * 1_024 * 1_024,
     ...
     * @param connection
     */
    public startRecording(connection: VoiceConnection, userFilter: (userId: string) => boolean = () => true): void {
        ...
        if (!this.writeStreams[guildId]) {
            ...
            let filteredListener = (userId: string) => {
                if (userFilter(userId)) listener(userId);
            }
